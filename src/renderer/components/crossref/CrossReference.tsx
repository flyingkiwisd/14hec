...
export default function CrossReference({ navigate }: CrossReferenceProps) {
  ...
  useEffect(() => {
    Promise.all([
      window.api.getAilments(),
      window.api.getZodiacSigns(),
      window.api.getPlanets(),
      window.api.getPreparations()
    ]).then(([a, z, p, pr]) => {
      setAilments(a)
      setSigns(z)
      setPlanets(p)
  ...
  const runQuery = async () => {
    ...
    const [recResults, avoidRes] = await Promise.all([
      window.api.crossReference(params),
      window.api.crossReferenceContraindications(contraindicationParams)
    ])
              ...
              return (
                <div key={plantId} className="card">
                  <div className="flex justify-between items-start mb-3">
                    <button
                      onClick={() => navigate({ view: 'plant-detail', id: Number(plantId) })}
