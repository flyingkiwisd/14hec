...
export default function EntheogenicGuide({ navigate }: EntheogenicGuideProps) {
  ...
  useEffect(() => {
    window.api.getPlants({ category: 'entheogenic' }).then((plants) => {
      setEntheogenicPlants(plants)
    })
    window.api.getPlants({ category: 'both' }).then((plants) => {
      setEntheogenicPlants((prev) => [...prev, ...plants])
    })
  }, [])

  const loadPlantDetail = async (plantId: number) => {
    const detail = await window.api.getPlantById(plantId)
          ...
          {selectedPlant && (
            <div className="card-glow-celestial animate-fade-in">
              <div className="flex justify-between items-start mb-4">
                ...
                <button
                  onClick={() => navigate({ view: 'plant-detail', id: selectedPlant.id })}
