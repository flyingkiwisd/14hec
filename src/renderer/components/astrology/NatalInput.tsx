...
export default function NatalInput({ navigate }: NatalInputProps) {
  ...
  const generateMap = async () => {
    ...
    for (const placement of placements) {
      ...
      const detail = await window.api.getZodiacSignById(placement.signId as number)
      if (detail) {
        newResults.push({
          sign: detail,
          role: placement.role,
